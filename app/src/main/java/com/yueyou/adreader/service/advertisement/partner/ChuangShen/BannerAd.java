...
public class BannerAd {
    public static final String url = "http://openapi.toukeads.com/openapi/adservice/b.action";
        ...
        try {
            ...
            call.enqueue(new Callback() {
                ...
                public void onResponse(Call call, Response response) throws IOException {
                    ...
                    try {
                        BannerAdBean bannerAdBean = new Gson().fromJson(strResponse, BannerAdBean.class);
                        if (bannerAdBean != null && bannerAdBean.getAds() != null) {
                            ((Activity) context).runOnUiThread(() -> {
                                try {
                                    setFeedAdData(context, adContent, bannerAdBean, bannerAdBean.getAds().getImage(), viewGroup);
                                } catch (Exception e) {
                                    e.printStackTrace();
                                }
                            });
...
//        Log.e("spptag", "realUrl" + realUrl);
    ...
    private static void setFeedAdData(Context context, AdContent adContent, BannerAdBean bannerAdBean, String imgUrl, ViewGroup viewGroup) {
        View[] views = AdEvent.getInstance().adShowPre(adContent, viewGroup, "", "", "", imgUrl);
        if (imgUrl != null && views != null && views.length > 0) {
            AdEvent.getInstance().adShow(adContent, null, null);
            List<String> report = bannerAdBean.getAds().getShowReport().get(0);
            for (int i = 0; i < report.size(); i++) {
                adReport(report.get(i));
            }
        }
        if (views.length > 0) {
            views[0].setOnClickListener(v -> {
                AdEvent.getInstance().adClicked(adContent);
                List<String> clickReport = bannerAdBean.getAds().getClickReport();
                if (clickReport != null && clickReport.size() > 0) {
                    adReport(clickReport.get(0));
                }
                dealWithGotoUrl(context, bannerAdBean.getAds().getGotourl(), bannerAdBean.getAds().getWords(), bannerAdBean.getAds().getCreativeId());
            });
        }
    }
...
//                Log.d("spptag", "onResponse: " + strResponse);
    ...
    public static Map<String, String> getRequestMap(Context context, String adid, String appkey) {
        ...
        requestMap.put("thumbHeight", "180");
    ...
    public static String getSignature(Context context, String adid, String appkey) {
        String normalSign = Utils.format("posId=%s&thumbWidth=%s&thumbHeight=%s&platform=%s&device=%s&timestamp=%s&secret=%s",
                adid, "1200", "180", "2", Widget.getDeviceId(context), (int) (System.currentTimeMillis() / 1000) + "", appkey);
